RSpec.describe Podling::EpisodesController, type: :request do
  ...
  describe '#new' do
    before { get new_episode_path }

    it { is_expected.to be_successful }
  end

  describe '#create' do
    let(:do_create) { post episodes_path, params: params }
    let(:audio) do
  ...
  describe '#edit' do
    before { get edit_episode_path(episode) }
  ...
  describe '#update' do
    let(:do_update) do
      patch episode_path(episode), params: params
    end
  ...
  describe '#destroy' do
    let(:do_destroy) { delete episode_path(episode) }
    let(:episode) { published_episode.tap(&:save) }
